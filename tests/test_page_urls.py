...
def test_page_urls(client):
    ...
    assert response.status_code==200
    response = client.get(url_for('member_page'), follow_redirects=True)
    assert response.status_code==200

    # Edit User Profile page
    response = client.get(url_for('user_profile_page'), follow_redirects=True)
    assert response.status_code==200
    ...
    assert response.status_code==200
    response = client.get(url_for('admin_page'), follow_redirects=True)
    assert response.status_code==200
